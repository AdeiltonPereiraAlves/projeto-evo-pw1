...
export const validarRegistroVoluntario =  () => {
  return [
    ...
    // body("cpf")
    //   .isString().withMessage("O CPF é obrigatório e precisa ser válido."),
    ...
    //   .isString().withMessage("A disponibilidade é obrigatória.")
    //   .isIn(["full-time", "part-time", "horários específicos"]).withMessage("A disponibilidade deve ser 'full-time', 'part-time' ou horários específicos."),

    // // Senha: deve ter entre 6 e 20 caracteres
    // body("senha")
    //   .isString().withMessage("A senha é obrigatória.")
    //   .isLength({ min: 6, max: 20 }).withMessage("A senha precisa ter entre 6 e 20 caracteres."),
